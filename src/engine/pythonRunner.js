const COMPILER_URL = 'https://godbolt.org/api/compiler/python314/compile'
...
export async function runPython(pythonCode) {
  try {
    ...
    if (!res.ok) {
      return { success: false, output: '', errors: `Python server error: ${res.status} ${res.statusText}` }
    }
    ...
    return { success: false, output: '', errors: `Python runner error: ${e.message}` }
  }
}
